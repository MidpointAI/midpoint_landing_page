  ...
  return (
    ...
    >
      ...
      <Cell left={REQ_LEFT} width={REQ_W} top={rowY(0) - ROW_H / 2}>
        <span className="text-[16px] text-foreground whitespace-nowrap leading-none">Requirements</span>
      </Cell>
      ...
      <div key={activeIndex} className="absolute" style={{ left: DET_LEFT, top: PAD }}>
        {coverage.details.map((d, i) => (
          ...
          >
            <span className="text-[16px] text-foreground whitespace-nowrap">{d.label}</span>
            {d.value && (
              <span
                className={`text-[16px] whitespace-nowrap ${
                  d.missing ? "text-foreground" : "text-primary"
                }`}
              >
                {d.value}
              </span>
            )}
            {d.check && <CheckIcon className="h-5 w-5 text-primary" strokeWidth={2.5} />}
            {d.missing && !d.value && (
              <span className="text-[16px] text-foreground whitespace-nowrap">Missing</span>
            )}
          </motion.div>
        ))}
      </div>
    ...
    >
      ...
      >
        <g
          key={activeIndex}
          fill="none"
          stroke={stroke} className="text-foreground/55"
          strokeWidth={1}
          strokeLinecap="round"
      ...
      >
        <span className="text-[15px] text-foreground whitespace-nowrap leading-none font-medium">
          Requirements
        </span>
      </div>
      ...
      <div key={activeIndex} className="absolute inset-0 pointer-events-none">
        {coverage.details.map((d, i) => (
          ...
          >
            <span className="text-[11px] text-foreground leading-tight pr-2">
              {d.label}
            </span>
            <div className="flex items-center gap-1 flex-shrink-0">
              ...
              {d.missing && !d.value && (
                <span className="text-[11px] text-foreground whitespace-nowrap leading-tight">
                  Missing
                </span>
              )}
            </div>
